...
-- Create certificate storage policies for service providers
...
-- Drop existing policy that depends on is_approved column
DROP POLICY IF EXISTS "Service providers are viewable by everyone when approved" ON public.service_providers;
...
-- Drop the old is_approved column
ALTER TABLE public.service_providers DROP COLUMN IF EXISTS is_approved;

-- Add new computed approval status that requires certificate approval
ALTER TABLE public.service_providers 
...
-- Recreate the policy with the new column
...
-- Create admin access policy for certificates storage
CREATE POLICY "Admins can view all certificates" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'certificates' AND public.has_role(auth.uid(), 'admin'::text));